...
**Phase 6 progress on the four items above:**

1. Google Maps `<iframe>` embeds added to both Contact location cards, "Open in maps" link kept alongside each. Done.
2. Named referral partners — still blocked on Dr. Kopp confirming the actual partner list (§2). Not started.
3. Condition leaf icons carried through: `leaf-tbi`/`leaf-stroke`/`leaf-concussion`/`leaf-neurobehavioral` on the brain-injury-rehabilitation managed-list, `leaf-cognitive`+`leaf-neurobehavioral` added to that page's "Targeted support for memory and behavior" card and to the FAQ's "What conditions do you treat?" question, `leaf-concussion` on the concussion-management Symptoms heading. Done.
4. Home hero marquee built. Done.

**One change made outside the four items above, superseding an earlier §14 decision:** Referrals now gets a hero photo (`hero-village`), reversing the "utility pages skip the hero photo" rule from §14 for this page specifically — the community-connections framing reads better with the warmth of a photo than the other, more transactional utility pages (FAQ, Insurance, Contact). FAQ, Insurance & Billing, Contact, Privacy, and Accessibility keep `.page-hero.no-media` as originally decided.

Also fixed in the same pass, undocumented until now: `lifestyle-medicine.html` and `physiatry-pmr.html` were both using the same `care-planning` stock photo in their quote-block figure — swapped to `family-kitchen` and `support-consult` respectively so no two service pages repeat a photo.
...
| **6. Systems** ✅ | Form live, full schema graph, sitemap/robots/llms.txt, 404, privacy, accessibility statement | Formspree wired (`xwlegyyw`) and verified with the existing AJAX handler; `sitemap.xml`/`robots.txt`/`llms.txt` added to `src/static/` and building into `docs/`; JSON-LD across all 17 pages parses as valid JSON and covers every type §9 calls for (`MedicalClinic`, `Physician`, `FAQPage`, `MedicalWebPage`/`MedicalCondition`/`Service` on service pages, `ContactPage`, `AboutPage`, `BreadcrumbList` everywhere) — **Rich Results Test itself still needs running against the live URLs once deployed, since it needs a reachable page, not a local file** |
